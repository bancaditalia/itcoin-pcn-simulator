...
def _add_swap_latencies_to_blockchain_df(blockchain_df: pd.DataFrame) -> pd.DataFrame:
  prepare_htlcs = blockchain_df[blockchain_df[COLS.TX_TYPE].str.contains("PREPARE_HTLC")].copy()
  claim_htlcs = blockchain_df[blockchain_df[COLS.TX_TYPE].str.contains("CLAIM_HTLC")].copy()
  prepare_htlcs[COLS.SWAP_DISAMBIGUATE_IDX.value] = prepare_htlcs.groupby([COLS.TX_SENDER, COLS.TX_RECEIVER, COLS.TX_AMOUNT]).cumcount() + 1
  claim_htlcs[COLS.SWAP_DISAMBIGUATE_IDX.value] = claim_htlcs.groupby([COLS.TX_SENDER, COLS.TX_RECEIVER, COLS.TX_AMOUNT]).cumcount() + 1

  columns_of_interest = [COLS.TX_TYPE, COLS.TX_SENDER, COLS.TX_RECEIVER, COLS.TX_AMOUNT, COLS.BLOCK_HEIGHT, COLS.SWAP_DISAMBIGUATE_IDX]
  ...
  merged[COLS.SWAP_LATENCY.value] = (merged[COLS.BLOCK_TIME] - merged[COLS.TX_START_TIME]).apply(lambda x: round(x, 2))
  return merged

def get_swap_latencies_from_blockchain_output(blockchain_output: str | os.PathLike) -> pd.core.series.Series:
