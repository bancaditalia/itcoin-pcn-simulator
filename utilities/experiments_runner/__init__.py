...
def select_rebalancing_mode(rebalancing_mode: RebalancingMode) -> Tuple[int, int, int]:
    if rebalancing_mode == RebalancingMode.FULL:
        return 1, 1, 1
    elif rebalancing_mode == RebalancingMode.REV:
        return 1, 1, 0
    elif rebalancing_mode == RebalancingMode.NONE:
        return 0, 0, 0
...
def run_all_simulations(
    ...
    capacities_in_a_topologies_seed_dir = [
        float(
            re.search('capacity-([0-9]*\.[0-9]*)',
                    os.path.basename(os.path.normpath(str(f))),
            re.IGNORECASE).group(1)
        )
        for f in a_topologies_seed_dir.iterdir() if f.is_dir()
    ]
    ...
    for block_congestion_rate, block_size, capacity, num_processes, seed, simulation_end, submarine_swap_threshold, rebalancing_mode, use_known_path, sync, topology_type, tps, tps_cfg in itertools.product(
        ...
        with open(os.path.join(results_dir,simulation_result_filename), 'w') as simulation_result_file:
