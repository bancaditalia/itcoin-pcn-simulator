...
def setup_topology_directories(
    dir_name: str, topology_type: TopologyType
) -> tuple[Path, Path]:
    """
    ...
    topologies_dir = (
        cloth_root_dir / "experiments" / "2024_COMCOM" / dir_name / topology_type.value
    )
...
def run_experiment_1() -> None:
    ...
    for topology_type in TopologyType:
        cloth_root_dir, topologies_dir = setup_topology_directories(
            "topologies", topology_type
        )
        generate_topologies(
            cloth_root_dir,
            topologies_dir,
...
def run_experiment_2() -> None:
    ...
    for topology_type in TopologyType:
        ...
        results = run_all_simulations(
            ...
            tpss=None,
            tps_cfgs=str(
                cloth_root_dir / "experiments" / "2024_COMCOM" / "PCN_load.txt"
            ),
            cleanup=False,
        )
